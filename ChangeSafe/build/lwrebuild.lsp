...
(let ((source-directory (and *load-pathname*
  ...
  (labels ((source-file (subdirectory name &optional (type "lsp"))
    ...
    ;; (load (source-file '("lisp-server-pages" "plist") "package" "lsp"))
    ;; (load (source-file '("lisp-server-pages" "htmlgen") "package" "cl"))

    ;; I wanted to use the lispworks make, but it seems to
    ;; be buggy.  At least I can repair any bugs in the MK defsystem.
