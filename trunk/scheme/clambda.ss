;;; Common Lisp lambda lists for Chez Scheme.
;;; Copyright 2008, Joe Marshall

;;; Section 3.4.1 specifies the syntax of an ordinary lambda list.


;;; Customization variables.  You should modify these to suit your needs.

;;; DEFAULT-KEY-VALUE
;;;  Keyword arguments will be bound to this value if they are not
;;; supplied and no init form is specified.  Common Lisp uses NIL.
(define (default-key-value) (string->symbol "#<Default KEY value>")) 

;;; DEFAULT-OPTIONAL-VALUE
;;;   Optional arguments are  initially bound to this value if no
;;; init form is specified.  Common Lisp uses NIL.  MIT-Scheme used to
;;; make the variable be unassigned, but now assigns it to the special
;;; token `#!default'
(define default-optional-value
  (let ((default-object (string->symbol "#<Default OPTIONAL value>")))
    (lambda () default-object)))

(define (default-object? thing) (eq? thing (default-optional-value)))

;;; DEFAULT-AUX-VALUE
;;;   Auxiliary variables are initially bound to this value if no
;;; init form is specified.  Common Lisp uses NIL.
(define (default-aux-value) (string->symbol "#<Default AUX value>"))


;;; End of customization variables.

;;; UNASSIGNED-KEY-VALUE
;;;  Keyword variables are temporarily bound to this value during
;;; keyword parsing.  You should not normally encounter it.
(define unassigned-key-value (string->symbol "#<UNASSIGNED-KEY>"))


;;; MIT-LAMBDA
;;;   MIT-Scheme lambda lists.  MIT-Scheme has optional arguments indicated
;;; by the token `#!optional'.  Rest arguments can be indicated by the token
;;; `#!rest' or by using a `dotted tail'.  We extend this by allowing either
;;; `&optional' or `#!optional' as the marker and `&rest', `#!rest' or a
;;; dotted tail as the rest argument marker.
...
(define-syntax mit-lambda-helper
  (syntax-rules (&optional &rest |#!optional| |#!rest|)
    ...
    ((mit-lambda-helper "emit-cases" unassigned-value function full-arglist body cases)
     (let ((unassigned-value (default-optional-value))
           (function (lambda full-arglist . body)))
       (case-lambda . cases)))))

;;; Handle &aux variables.
...
(defun hairy (a b &optional c (d 10) (e 42 e-supplied?) &rest keyargs &key f (g 69) ((hkey h) 11 h-supplied?))
  (vector a b c d (list e e-supplied?) keyargs f g (list h h-supplied?)))
