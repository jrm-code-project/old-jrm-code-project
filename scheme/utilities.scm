...
(define-integrable (count-if predicate list)
  (fold-left (lambda (total element)
...
(declare (integrate square))
(define (square x)
  (* x x))

;; Data is a list of CONS cells where the
;; CAR is the x-coordinate and the CDR is the y-coordinate
